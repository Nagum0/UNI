...
|
    T_ID T_ASSIGN expression
    {
        ...
        std::stringstream ss;
	if (symbol_table[$1].typ == real) {
            ss << $3.code
	    << "mov [" << symbol_table[$1].label << "], eax\n" 
	    << "mov [" << symbol_table[$1].label << " + 1], ebx\n";
        }
        else {
            ss << $3.code;
...
|
    ...
    {
        if($1.typ == natural && $3.typ == natural) {
            std::stringstream ss;
            ss << $1.code;
            ss << "push eax" << std::endl;
            ss << $3.code;
            ...
            semantic_error(@1.begin.line, "Type error in '+' operation: operands not the same type.");
        }
    }
|
