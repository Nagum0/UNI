...
|
    T_ID T_ASSIGN expression T_SEMICOLON
    {
        ...
        if (symbol_table[$1].locked)
            semantic_error(@1.begin.line, "Variable is locked.");   
...
|
    T_READ T_OPEN T_ID T_CLOSE T_SEMICOLON
    {
        ...
        if (symbol_table[$3].locked)
            semantic_error(@3.begin.line, "Variable is locked.");
...
|
    T_LOCK T_ID T_SEMICOLON
    {
        if (symbol_table.count($2) == 0)
            semantic_error(@2.begin.line, "Undeclared variable.");
        
        if (symbol_table[$2].locked)
            semantic_error(@2.begin.line, "Variable is locked.");

        symbol_table[$2].locked = true;
    }
|
    T_UNLOCK T_ID T_SEMICOLON
    {
        if (symbol_table.count($2) == 0)
            semantic_error(@2.begin.line, "Undeclared variable.");
        
        if (!symbol_table[$2].locked)
            semantic_error(@2.begin.line, "Variable is already unlocked.");

        symbol_table[$2].locked = false;
    }
|
    T_EQUAL expression expression T_SEMICOLON
    {
        if ($2.typ != $3.typ)
            semantic_error(@2.begin.line, "Type error.");

        std::string true_label = next_label();
        std::string func_to_call = $3.typ == boolean ? "write_boolean" : "write_unsigned";
