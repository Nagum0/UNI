...
-- --> Azért nem látható a táblatér, mert az IOT-nek nincs szegmense, csak a hozzá tartozó indexnek és 
-- --> esetleges OVERFLOW-nak van szegmense.    
-- --------------------------------------------------------------------------------
SELECT owner, table_name, iot_name, iot_type, tablespace_name FROM dba_tables 
WHERE owner='NIKOVITS' AND iot_type = 'IOT';     -- iot_type = 'IOT' innen tudjuk, hogy index-szervezett
...
WHERE table_owner='NIKOVITS' AND index_type LIKE '%IOT%TOP%';  -- 'IOT - TOP' vannak benne szóközök, ezért a LIKE
...
-- --> Az objektumokat a nevük (és persze tulajdonosuk) alapján találjuk meg a DBA_OBJECTS-ben.

-- 5.
-- Adjuk meg a fenti táblák túlcsordulási részeit (ha van).
-- --------------------------------------------------------------------------------
SELECT owner, table_name, iot_name, iot_type FROM dba_tables 
WHERE owner='NIKOVITS' AND iot_type = 'IOT_OVERFLOW';   -- iot_type = 'IOT_OVERFLOW' innen tudjuk, hogy ez overflow rész
...
-- EXECUTE iot_size('nikovits', 'cikk_iot2');
...
-- Az alábbi megadja a szegmens ténylegesen lefoglalt blokkjainak számát:
...
-- WHERE segment_type LIKE 'TABLE%PARTITION'  -- TABLE PARTITION vagy TABLE SUBPARTITION
...
-- EXECUTE empty_blocks('nikovits','emp');
-- -- Még az alábbira is gyorsan kell lefutnia, ha hatékony programot írtak.
-- EXECUTE empty_blocks('nikovits','customers');
-- -- Az alábbi tábla csak az ullmanon van meg, erre már futhat 15-20 másodpercet is.
...
-- Az output formája ilyen legyen (YES/NO):
-- Clustered: YES Partitioned: NO IOT: NO
