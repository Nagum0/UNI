...
|
    T_ID T_ASSIGN expression
    {
        ...
        if (symbol_table[$1].typ == t_real)
        {
            ss << $3.code
               << "mov [" << symbol_table[$1].label << "], eax\n" 
               << "mov [" << symbol_table[$1].label << " + 1 * 4], ebx\n";
        }
        else {
            ss << $3.code;
...
|
    ...
    {
        std::stringstream ss;
        size_t comma_pos = $1.find(",");
        std::string int_part = $1.substr(0, comma_pos);
        std::string frac_part = $1.substr(comma_pos + 1);
        ss << "mov eax, " << int_part << "\nmov ebx, " << frac_part << "\n";
        $$ = expression(t_real, ss.str());
    }
|
    T_INT T_OPEN T_ID T_CLOSE
    {
        ...
        if (symbol_table[$3].typ != t_real) {
        	semantic_error(@3.begin.line, "Type error.");
        }
        
        std::stringstream ss;
        ss << "mov eax, [" << symbol_table[$3].label << "]\n";
        $$ = expression(natural, ss.str());
...
|
    T_FRAC T_OPEN T_ID T_CLOSE
    {
        ...
        std::stringstream ss;
        ss << "mov eax, [" << symbol_table[$3].label << " + 1 * 4]\n";
        $$ = expression(natural, ss.str());
...
|
    T_ID
    {
        ...
        if (typ == t_real) {
            ss << "mov eax, [" << symbol_table[$1].label << "]\n"
               << "mov ebx, [" << symbol_table[$1].label << " + 1 * 4]\n";
        }
        else {
...
|
    ...
    {
        if ($1.typ != $3.typ && ($1.typ != natural || $1.typ != t_real)) {
