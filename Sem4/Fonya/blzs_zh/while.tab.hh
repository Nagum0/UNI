...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    T_TIME = 277,                  // T_TIME
    T_HOUR = 278,                  // T_HOUR
    T_MINUTE = 279,                // T_MINUTE
    T_ID = 280,                    // T_ID
    T_NUM = 281,                   // T_NUM
    T_REAL = 282,                  // T_REAL
    T_REAL_LITERAL = 283,          // T_REAL_LITERAL
    T_INT = 284,                   // T_INT
    T_FRAC = 285,                  // T_FRAC
    T_OR = 286,                    // T_OR
    T_AND = 287,                   // T_AND
    ...
    T_LESSEQ = 291,                // T_LESSEQ
    T_GREATEREQ = 292,             // T_GREATEREQ
    T_ADD = 293,                   // T_ADD
    T_SUB = 294,                   // T_SUB
    T_MUL = 295,                   // T_MUL
    T_DIV = 296,                   // T_DIV
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_T_BEGIN = 4,                           // T_BEGIN
        S_T_END = 5,                             // T_END
        S_T_BOOLEAN = 6,                         // T_BOOLEAN
        S_T_NATURAL = 7,                         // T_NATURAL
        S_T_READ = 8,                            // T_READ
        S_T_WRITE = 9,                           // T_WRITE
        S_T_IF = 10,                             // T_IF
        S_T_THEN = 11,                           // T_THEN
        ...
        S_T_TIME = 22,                           // T_TIME
        S_T_HOUR = 23,                           // T_HOUR
        S_T_MINUTE = 24,                         // T_MINUTE
        S_T_ID = 25,                             // T_ID
        S_T_NUM = 26,                            // T_NUM
        S_T_REAL = 27,                           // T_REAL
        S_T_REAL_LITERAL = 28,                   // T_REAL_LITERAL
        S_T_INT = 29,                            // T_INT
        S_T_FRAC = 30,                           // T_FRAC
        S_T_OR = 31,                             // T_OR
        S_T_AND = 32,                            // T_AND
        S_T_EQ = 33,                             // T_EQ
        S_T_LESS = 34,                           // T_LESS
        S_T_GREATER = 35,                        // T_GREATER
        S_T_LESSEQ = 36,                         // T_LESSEQ
        S_T_GREATEREQ = 37,                      // T_GREATEREQ
        S_T_ADD = 38,                            // T_ADD
        S_T_SUB = 39,                            // T_SUB
        S_T_MUL = 40,                            // T_MUL
        S_T_DIV = 41,                            // T_DIV
...
switch (yykind)
    ...
    // Implementation of make_symbol for each token kind.
      ...
      static
      symbol_type
      make_T_NATURAL (location_type l)
      {
        return symbol_type (token::T_NATURAL, std::move (l));
      }
#else
      static
      symbol_type
      make_T_NATURAL (const location_type& l)
      {
        return symbol_type (token::T_NATURAL, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      static
      symbol_type
      make_T_TIME (std::string v, location_type l)
      {
        return symbol_type (token::T_TIME, std::move (v), std::move (l));
      }
#else
      static
      symbol_type
      make_T_TIME (const std::string& v, const location_type& l)
      {
        return symbol_type (token::T_TIME, v, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_T_HOUR (location_type l)
      {
        return symbol_type (token::T_HOUR, std::move (l));
      }
#else
      static
      symbol_type
      make_T_HOUR (const location_type& l)
      {
        return symbol_type (token::T_HOUR, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_T_MINUTE (location_type l)
      {
        return symbol_type (token::T_MINUTE, std::move (l));
      }
#else
      static
      symbol_type
      make_T_MINUTE (const location_type& l)
      {
        return symbol_type (token::T_MINUTE, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      static
      symbol_type
      make_T_NUM (std::string v, location_type l)
      {
        return symbol_type (token::T_NUM, std::move (v), std::move (l));
      }
#else
      static
      symbol_type
      make_T_NUM (const std::string& v, const location_type& l)
      {
      ...
      static
      symbol_type
      make_T_REAL_LITERAL (std::string v, location_type l)
      {
        return symbol_type (token::T_REAL_LITERAL, std::move (v), std::move (l));
      }
#else
      static
      symbol_type
      make_T_REAL_LITERAL (const std::string& v, const location_type& l)
      {
        return symbol_type (token::T_REAL_LITERAL, v, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      static
      symbol_type
      make_T_FRAC (location_type l)
      {
        return symbol_type (token::T_FRAC, std::move (l));
      }
#else
      static
      symbol_type
      make_T_FRAC (const location_type& l)
      {
        return symbol_type (token::T_FRAC, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      static
      symbol_type
      make_T_GREATER (location_type l)
      {
      ...
      static
      symbol_type
      make_T_GREATER (const location_type& l)
      {
      ...
      static
      symbol_type
      make_T_LESSEQ (location_type l)
      {
        return symbol_type (token::T_LESSEQ, std::move (l));
      }
#else
      static
      symbol_type
      make_T_LESSEQ (const location_type& l)
      {
        return symbol_type (token::T_LESSEQ, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_T_GREATEREQ (location_type l)
      {
        return symbol_type (token::T_GREATEREQ, std::move (l));
      }
#else
      static
      symbol_type
      make_T_GREATEREQ (const location_type& l)
      {
        return symbol_type (token::T_GREATEREQ, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
