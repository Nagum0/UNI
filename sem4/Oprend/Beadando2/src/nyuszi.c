...
nyuszi_t* nyuszi_list_winner(nyuszi_list_t* nyuszik) {
    ...
    nyuszi_t* winner = malloc(sizeof(nyuszi_t));
    if (winner == NULL) {
        fprintf(stderr, "Error while appending nyuszi to nyuszik...\n");
        exit(1);
    }
    
    nyuszi_set_name(winner, "NYUSZI");
    nyuszi_set_eggs(winner, 0);
    nyuszi_set_poem(winner, "VERS");
    ...
    for (size_t i = 0; i < nyuszik->len; ++i) {
        ...
        if (nyuszi->eggs > winner->eggs) {
            nyuszi_set_name(winner, nyuszi->name);
            nyuszi_set_eggs(winner, nyuszi->eggs);
            nyuszi_set_poem(winner, nyuszi->poem);
        }
    }
