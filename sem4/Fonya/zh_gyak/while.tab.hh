  ...
  private:
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_T_SHIFT = 28,                          // T_SHIFT
        S_T_POWER = 29,                          // T_POWER
        S_T_ASSERT = 30,                         // T_ASSERT
...
switch (yykind)
    ...
    // Implementation of make_symbol for each token kind.
      ...
      static
      symbol_type
      make_T_SHIFT (location_type l)
      {
        return symbol_type (token::T_SHIFT, std::move (l));
      }
#else
      static
      symbol_type
      make_T_SHIFT (const location_type& l)
      {
        return symbol_type (token::T_SHIFT, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_T_POWER (location_type l)
      {
        return symbol_type (token::T_POWER, std::move (l));
      }
#else
      static
      symbol_type
      make_T_POWER (const location_type& l)
      {
        return symbol_type (token::T_POWER, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
