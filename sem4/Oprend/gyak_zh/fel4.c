...
int main(int argc, char *argv[])
{
    ...
    if (gy1)
    {
        ...
        if (gy2)
        {
            ...
            struct msg to_receive2;
            msgrcv(msgQ, &to_receive2, sizeof(to_receive2) - sizeof(long), 6, 0);
            pt("info: %s", to_receive2.buf);

            szemafor_muvelet(sem, -1);
            int logfile_fd = open("logfile.txt", O_WRONLY | O_CREAT | O_APPEND, S_IRUSR | S_IWUSR);
            char buf[200];
            sprintf(buf, "info: %s\ninfo: %s\n\n", to_receive.buf, to_receive2.buf);
